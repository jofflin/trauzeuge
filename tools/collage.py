#!/usr/bin/env python3
"""Druck-Collage (A4, 300 dpi) aus src/photos + captions.json. Lokales Tool, nicht Teil des Deploys.

  pip install pillow            (einmalig, z.B. in einem venv)
  python tools/collage.py                 -> print/collage-a4-quer.jpg/.pdf (+ HTML)
  python tools/collage.py --portrait      -> print/collage-a4-hoch.jpg/.pdf
  python tools/collage.py --no-render     (nur HTML, ohne headless Chrome)
"""
...
def load_photos():
    ...
    out = []
    for p in sorted(x for x in (ROOT / "src" / "photos").iterdir() if x.suffix.lower() in {".jpg", ".jpeg", ".png"}):
        meta = {"caption": "Aus dem Archiv", "date": "undatiert", "order": 99999999}; meta.update(captions.get(p.name, {}))
